...
function createCactusWithRandomIntervalAndPosition() {
  if (!gameIsOver) {
    // Créer la div pour le cactus
    const cactus = document.createElement("div");
    cactus.classList.add("cactus");

    // Créer l'image pour le cactus
    const cactusImage = document.createElement("img");
    cactusImage.src = "/assets/images/cactus.png"; // Remplacez par le chemin de votre image
    cactus.appendChild(cactusImage);

    // Ajouter du style pour réduire la taille de l'image
    cactusImage.style.width = "100px"; // Ajustez la largeur selon vos besoins
    cactusImage.style.height = "auto"; // Laisser la hauteur s'ajuster proportionnellement
    ...
    const randomInterval = Math.floor(Math.random() * 2000) + 1250;
